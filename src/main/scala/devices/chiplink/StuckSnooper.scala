...
class StuckSnooper(uFn: Seq[TLClientPortParameters] => TLClientPortParameters)(implicit p: Parameters) extends LazyModule
{
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val divertprobesnext = Wire(Bool())
    val divertprobes = RegInit(divertprobesnext, true.B)
    divertprobesnext := divertprobes && ~(in1.a.fire() && (in1.a.bits.opcode === TLMessages.AcquireBlock || in1.a.bits.opcode === TLMessages.AcquirePerm))
    val bypass_c = bypass || divertprobes
